...
class AddSquareBicolorWatchSeeder extends Seeder
{
    public function run(): void
    {
        $folder = '011-square-bicolor-paved';
        ...
        foreach ($requiredImages as $image) {
            ...
            if (! is_file($path)) {
                throw new \RuntimeException(
                    'Missing image: '.$path.'. Extract the square-bicolor model assets before running this seeder.'
                );
            }
        }
        ...
        if ($existing !== null) {
            $existing->name = '41 mm · Carrée bicolore, cadran pavé';
            $existing->description = 'Boîtier carré, cadran pavé à chiffres romains et finition bicolore avec accents couleur or jaune. Le sertissage en moissanite VVS couleur D se prolonge sur le bracelet intégré.';
