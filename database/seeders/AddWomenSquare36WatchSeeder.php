...
class AddWomenSquare36WatchSeeder extends Seeder
{
    public function run(): void
    {
        $folder = '013-women-square-36';
        ...
        foreach ($requiredImages as $image) {
            ...
            if (! is_file($path)) {
                throw new \RuntimeException(
                    'Missing image: '.$path.'. Extract the women-square-36 model assets before running this seeder.'
                );
            }
        }
        ...
        $catalog['next_watch_id'] = max((int) ($catalog['next_watch_id'] ?? 1), 14);
